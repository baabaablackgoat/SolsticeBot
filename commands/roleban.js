...
module.exports = function(bot,msg,args,options) {
    if (!options.settings.roleban.enabled) {
        msg.channel.send("Rolebans have not been enabled. Ask the bot owner to enable them in settings.js.");
        return;
    }
    let mentions = getIDfromMention(msg);
    if (mentions.length < 1) {
        msg.channel.send("You asked me to roleban someone, but you didn't provide a valid mention.");
        return;
    }
    for (let i=0;i<mentions.length;i++){
        if (mentions[i] === msg.author.id) {
            ...
            msg.channel.send("You successfully rolebanned yourself. Congratulations, great work. http://i.giphy.com/9uoYC7cjcU6w8.gif");
            return;
        }
        let access = compareUserAccess(msg, msg.author.id, mentions[i], false, true);
        if(access === "err_role") {
            msg.channel.send("You cannot roleban this user. They have a equal or higher role than you.");
            return;
            ...
            msg.guild.members.get(mentions[i]).addRole(options.settings.roleban.role);
            msg.channel.send(msg.guild.members.get(mentions[i]).displayName + " has been rolebanned.");
        } else {
            msg.channel.send("`Internal Error, bug the bot owner or @TrueMGF#0114 about this`");
